...
class GasConduction(NumericalModel):
	...
	g1 = FieldGroup([fluidName, p, T1, T2, d], label = 'Inputs')
	
	inputs = SuperGroup([g1])
	###########
	...
	r2 = FieldGroup([Kn, qDot], label = 'Heat transfer')
	...
	def compute(self):
		...
		self.qDot = state.cond / self.d * np.abs(self.T1 - self.T2)
