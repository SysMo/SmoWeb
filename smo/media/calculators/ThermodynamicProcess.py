...
class ThermodynamicProcess(NumericalModel):
	...
	startingState = FieldGroup([fluidName, stateVariable1, p1, T1, rho1, h1, s1, q1, stateVariable2, p2, T2, rho2, h2, s2, q2], label = 'Starting state')
	...
	finalState = FieldGroup([transitionType, final_StateVariable1, final_p1, final_T1, final_rho1, final_h1, eta], label = 'Final state')
	
	inputs = SuperGroup([startingState, finalState])
	...
	results = SuperGroup([initialStateResults, finalStateResults], label = "Results")
	...
	def compute(self):
		...
		if (self.transitionType == 'S'):
			...
			idealWork = finalState.h - initState.h
			realWork = idealWork / self.eta
